...
import { downloadBlob } from '../../shared/utils/export-file.util';
import {
  CustomerDueReport,
  CustomerSalesReport,
  LowStockReport,
...
export class ReportsComponent implements OnInit {
  ...
  readonly cards: ReportCard[] = [
    ...
    { title: 'Sales Invoice Report', description: 'Invoice-level sales lines by date, customer, warehouse, and status.', category: 'Sales', route: '/reports/view/sales-detail', internal: true },
    { title: 'Sales Return Report', description: 'Sales return activity by customer, invoice, product, warehouse, and status.', category: 'Sales', route: '/reports/view/sales-returns', internal: true },
    { title: 'Sales By Product', description: 'Best-selling products ranked by sold quantity and gross sales.', category: 'Sales', route: '/reports/view/top-selling-products', internal: true },
    ...
    { title: 'Purchase By Product', description: 'Product-wise purchased quantity, purchase value, return quantity, and net quantity.', category: 'Purchase', route: '/reports/view/purchase-by-product', internal: true },
    ...
    { title: 'Negative Stock Report', description: 'Current product/warehouse balances below zero.', category: 'Inventory', route: '/reports/view/negative-stock', internal: true },
  ...
  readonly detailTitles: Record<DetailReportType, string> = {
    ...
    'sales-detail': 'Sales Invoice Report',
    'sales-returns': 'Sales Return Report',
  ...
  get canPrintReport(): boolean {
    return this.hasPermission('REPORT_EXPORT_PDF') || this.hasPermission('REPORT_EXPORT');
  }
  ...
  get showStatusFilter(): boolean {
    return ['sales-detail', 'sales-returns', 'purchase-detail', 'purchase-by-product', 'stock-transfers'].includes(this.reportType);
  }
  ...
  get showCustomerFilter(): boolean {
    return ['sales-summary', 'sales-detail', 'sales-returns', 'customer-sales'].includes(this.reportType);
  }

  get showSupplierFilter(): boolean {
    return ['purchase-summary', 'purchase-detail', 'purchase-by-product', 'supplier-purchases', 'purchase-returns'].includes(this.reportType);
  }

  get showProductFilter(): boolean {
    return ['sales-detail', 'sales-returns', 'top-selling-products', 'purchase-by-product', 'stock', 'stock-movements', 'low-stock', 'negative-stock'].includes(this.reportType);
  }

  get showWarehouseFilter(): boolean {
    return ['sales-detail', 'sales-returns', 'purchase-detail', 'purchase-by-product', 'top-selling-products', 'stock', 'stock-movements', 'low-stock', 'negative-stock', 'warehouse-stock-valuation', 'stock-transfers'].includes(this.reportType);
  }

  get showCategoryFilter(): boolean {
    return ['top-selling-products', 'purchase-by-product', 'stock', 'low-stock', 'negative-stock', 'warehouse-stock-valuation'].includes(this.reportType);
  }
  ...
  private reportRequest(): Observable<unknown> {
    ...
    if (this.reportType === 'sales-returns') return this.reportsService.getSalesReturns(this.filters);
    ...
    if (this.reportType === 'purchase-by-product') return this.reportsService.getPurchaseByProduct(this.filters);
    ...
    if (this.reportType === 'negative-stock') return this.reportsService.getNegativeStock(this.filters);
  ...
  private bindReport(report: unknown): void {
    ...
    if (this.reportType === 'sales-returns') {
      this.bindSalesReturns(report as SalesReturnReport);
      return;
    }
    ...
    if (this.reportType === 'purchase-by-product') {
      this.bindPurchaseByProduct(report as PurchaseByProductReport);
      return;
    }
    ...
    if (this.reportType === 'stock' || this.reportType === 'negative-stock') {
  ...
  private bindSalesReturns(report: SalesReturnReport): void {
    this.summaryCards = [
      { label: 'Return Count', value: report.returnCount, type: 'number' },
      { label: 'Return Amount', value: report.totalReturnAmount, type: 'currency', accent: 'accent-amber' }
    ];
    this.tableColumns = [
      { key: 'date', label: 'Date', type: 'datetime' },
  ...
  private bindPurchaseByProduct(report: PurchaseByProductReport): void {
    this.summaryCards = [
      { label: 'Quantity Purchased', value: report.totalQuantityPurchased, type: 'number' },
      { label: 'Gross Purchase', value: report.totalGrossPurchase, type: 'currency', accent: 'accent-blue' },
      { label: 'Return Qty', value: report.totalReturnQty, type: 'number', accent: 'accent-amber' },
      { label: 'Net Qty', value: report.totalNetQty, type: 'number', accent: 'accent-profit' }
    ];
    this.tableColumns = [
      { key: 'product', label: 'Product' },
      { key: 'sku', label: 'SKU' },
      { key: 'quantityPurchased', label: 'Purchased Qty', type: 'number' },
      { key: 'grossPurchase', label: 'Gross Purchase', type: 'currency' },
      { key: 'returnQty', label: 'Return Qty', type: 'number' },
      { key: 'netQty', label: 'Net Qty', type: 'number' }
    ];
    this.tableRows = this.asTableRows(report.rows);
  ...
  private exportReport(format: ReportExportFormat): void {
    this.reportsService.exportReport(this.reportType, this.filters, format).subscribe({
      next: blob => downloadBlob(blob, `${this.reportType}.${format === 'excel' ? 'xls' : format}`),
      error: () => this.error = 'Unable to export report.'
    });
  }
